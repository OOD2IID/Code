...
def train(trainloader, criterion, optimizer, model, writer, epoch, args):
    ...
    for i, (inputs_clean, targets_clean) in enumerate(trainloader):
        ...
        loss = criterion(logits_clean, targets_clean)
        ...
        losses_clean.update(loss.item(), inputs_clean.size(0))
...
def adv_train(trainloader, criterion, optimizer, model, writer, epoch, args):
    ...
    for i, (inputs_clean, targets_clean) in enumerate(trainloader):
        ...
        if args.attacker == 'PGD':
            inputs_adv = projected_gradient_descent(adv_generating_model, inputs_clean, eps, nb_iter=40,  norm = np.inf, eps_iter=0.01)
        '''
        ...
        loss = criterion(logits_adv, targets_clean)
        ...
        # adv
        acc, _ = accuracy(logits_adv, targets_clean, topk=(1, 5))
        ...
        progress_bar(i, len(trainloader), 'Epoch: %d | loss: %.3f | acc: %.3f '
         % (epoch, losses_combine.avg, top1_combine.avg))
