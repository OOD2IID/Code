...
def adv_train(trainloader, criterion, optimizer, model, writer, epoch, args):
    ...
    for i, (inputs_clean, targets_clean) in enumerate(trainloader):
        ...
        if args.dataset == 'IMAGENET':
            mask = targets_clean<100
            targets_clean= targets_clean[mask]
            inputs_clean= inputs_clean[mask]
        ...
        if args.attacker == 'PGD':
            inputs_adv = projected_gradient_descent(model, inputs_clean, eps, nb_iter=40,  norm = np.inf, eps_iter=0.01)
        else:
            inputs_adv = fast_gradient_method(model, inputs_clean, eps, clip_min= 0, clip_max= 1,ord=np.inf)    
        '''
        ...
        if i>10000 and args.dataset=='IMAGENET':
...
def validate(testloader, model, criterion, writer, epoch,dataset ='cifar10'):
