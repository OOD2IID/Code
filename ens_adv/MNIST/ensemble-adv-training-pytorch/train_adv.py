...
def main(args):
    ...
    data = ld_mnist(args.dataroot,batch_size=args.batch_size)
    ...
    model=PyNet(in_channels=1).to(device)
    ...
    for epoch in range(args.epochs):
        for batch_idx, (data, labels) in enumerate(train_loader):
            ...
            for i, m in enumerate(adv_models + [model]):
                ...
                x_advs[i] = projected_gradient_descent(m, data, eps, 0.01, 40, np.inf)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataroot', type=str, help='mnist data root')
    ...
    parser.add_argument('--batch_size', type=int, default=128, help='Size of training batches (default: 64)')
