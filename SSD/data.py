...
def MNIST(
    data_dir, batch_size=128, mode="base", normalize=True, norm_layer=None, size=28
):
    """
    mode: org | base | ssl
    """
...
def IMAGENET(
    data_dir, batch_size=25, mode="base", normalize=True, norm_layer=None, size=256
):
    """
    mode: org | base | ssl
    """
    ...
    transform_test = [transforms.Resize(size), transforms.CenterCrop(size), transforms.ToTensor()]

    if mode == "org":
        None
    elif mode == "base":
        transform_train = [transforms.Resize(size), transforms.CenterCrop(size), transforms.ToTensor()]
    elif mode == "ssl":
        transform_train = [
            transforms.RandomResizedCrop(size, scale=(0.2, 1.0)),
            transforms.RandomHorizontalFlip(),
