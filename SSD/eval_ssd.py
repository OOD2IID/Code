...
def plot(x,yin,yout):
    ...
    x1 = [num + 0.4 for num in x]
    x2 = [num - 0.4 for num in x]
    # creating the bar plot
    plt.bar(x1, yin, color ='blue',
            width = 0.4,label='in')
    plt.axhline(y=np.mean(yin), color='b', linestyle='-')
    plt.bar(x2, yout, color ='red',
            width = 0.4,label='out')
    plt.axhline(y=np.mean(yout), color='r', linestyle='-')
    plt.xlabel("samples")
    plt.ylabel("scores")
    plt.title('in-out scores')
    plt.legend()
    plt.savefig('in-out scores')
...
def main():
    ...
    #print('model accuracy ', knn(model, device, test_loader)[0])
    ...
    for d in ds:
        ...
        features_ood, _ = get_features(model.encoder, ood_loader,attack='PGD')
